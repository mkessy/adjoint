...
import { Chunk, DateTime, Effect, HashMap, pipe, Schema } from "effect"
...
describe("Graph Composition", () => {
  ...
  describe("transform() - Function composition", () => {
    it("should compose two transformations correctly", async () => {
      const sourceSchema = createTestSchemaNode("schema-1", "string-schema")
    ...
    it("should handle chain of transformations", async () => {
      const sourceSchema = createTestSchemaNode("schema-1", "string-schema")
      ...
      const composition = await Effect.runPromise(
        pipe(
          ...
          Effect.flatMap(Composition.transform<string, number, boolean>(strategy2))
        )
      )
  ...
  describe("Mathematical Properties", () => {
    it("should satisfy identity law: from(A).transform(id) ≡ from(A)", async () => {
      const sourceSchema = createTestSchemaNode("schema-1", "string-schema")
    ...
    it("should satisfy associativity: (f ∘ g) ∘ h ≡ f ∘ (g ∘ h)", async () => {
      const sourceSchema = createTestSchemaNode("schema-1", "string-schema")
      ...
      // (f ∘ g) ∘ h
      const leftAssoc = await Effect.runPromise(
        pipe(
          Composition.from<string>(sourceSchema),
          Composition.transform(f),
          Effect.flatMap(Composition.transform(g)),
          Effect.flatMap(Composition.transform(h))
        )
      )

      // f ∘ (g ∘ h) - conceptually the same final result
      const rightAssoc = await Effect.runPromise(
        pipe(
          Composition.from<string>(sourceSchema),
          Composition.transform(f),
          Effect.flatMap(Composition.transform(g)),
